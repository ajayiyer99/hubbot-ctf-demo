...
- **🛡️ Identity** *(the decisive cage — always)* — Microsoft Graph `PATCH /servicePrincipals { "accountEnabled": false }` → **204**, then `removePassword` revokes the leaked client secret. Disabling the service principal is a **continuous access evaluation** event, so CAE‑aware resources reject the token the agent already holds (**401** + claims challenge) instead of honouring it until it expires.
...
> **SAY:** "No human clicked anything — about **26–28 seconds** from the attempt to fully contained. Here's a nuance a security practitioner will appreciate: the Defender **alert** is `Medium`, but the **incident** is escalated to `High` — because this is a privileged AI workload identity with a *confirmed* injection. That's exactly how a real SOC prioritizes: you triage the incident, not the raw alert. And notice the cage **scaled to the attack** — a targeted VIP break‑the‑glass gets **identity + data**; a bulk PHI dump or prior‑auth fraud adds **network**; the smuggled patient‑safety attack adds the **compute/model kill switch**. We **disabled its identity and revoked the leaked secret** in Microsoft Graph, **stripped its PHI access** across RBAC, FHIR/Health Data Services, Purview and Key Vault, and **cut its network paths** at the WAF and Azure Firewall. Identity, data, network, compute: that's defense in depth."
...
- **"How fast is 'fast', and can I control it?"** — Default ≈ **15 s** detection + ~0.6 s per playbook step ≈ **caged in ~26–28 s** — **18 steps** (identity + data), **20** with the network plane, or **22** with the patient‑safety kill switch. Both are sliders in `⚙ Settings` (**Detection latency**, **Response pace**) — tune live to the room. (Benign prompts log near‑real‑time as Informational audit entries; only the security detection carries the full latency.)
